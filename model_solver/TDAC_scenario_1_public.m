...
while threads<=max_threads
    ...
    while (abs(R_prev-R)>=epsilon)
        ...
        i=1;
        while i<=n
            k=i:n; V(i)=(-1)^(n-(i-1))*(1/(prod(p(k,1)-1)));
            i=i+1;
        end
        R=dot(V,T)+tc;
        i=1;
        ...
        i=1;
        while i<=n
            p(i,1)=1/10*n/d*(threads-1)*f_avg_x+9/10*p(i,1);
            i=i+1;
        end
    end
    Time1=R;
    fprintf("\t%f\t%f", Time1, threads/Time1);
    if (print_avg_time_run_and_abort_prob)
        fprintf("\t%f\t%f", Time1/V(1),1-1/V(1));
    end
    ...
    while (abs(R_prev-R)>epsilon)
        ...
        i=1;
        while i<=n
            k=i:n; V(i)=(-1)^(n-(i-1))*(1/(prod(p(k,2)-1)));
            i=i+1;
        end
        R=dot(V,T)+tc;
        i=1;
        ...
        while i<=n
            ...
            while x<=d;
                ...
                k=1;
                while (k<=n)
                    f_avg_x=f_avg_x+access_probability(x,k,d,n,c) *f(k);
                    k=k+1;
                end
                sum=sum+access_probability(x,i,d,n,c)*f_avg_x;
                x=x+1;
            end
            p(i,2)=1/10*sum* (threads-1)+9/10*p(i,2);
            i=i+1;
        end
    end
    Time2=R;
    fprintf("\t%f\t%f", Time2, threads/Time2);
    if (print_avg_time_run_and_abort_prob)
        fprintf("\t%f\t%f", Time2/V(1),1-1/V(1));
    end    
        ...
        while i<=n
            ...
            while x<=d;
                ...
                while (k<=n)
                    ap1=access_probability(x,k,d,n,c);
                    f_avg_x_G1=f_avg_x_G1+ap1*f_G1(k);
                    ap2=access_probability(d-x+1,k,d,n,c);
                    f_avg_x_G2=f_avg_x_G2+ap2*f_G2(k);
                    k=k+1;
                end
                ap=access_probability(x,i,d,n,c);
                ...
                ap=access_probability(d-x+1,i,d,n,c);
                sum_G2_G1=sum_G2_G1+ap*f_avg_x_G1;
                sum_G2_G2=sum_G2_G2+ap*f_avg_x_G2;
    ...
    fprintf("\t%f\t%f", Time4, threads/Time4);
    if (print_avg_time_run_and_abort_prob)
        fprintf("\t%f\t%f", Time4/V(1),1-1/V(1));
    end    
  
    Throughput4(threads)=threads/Time4;
    Abort_probability4(threads)=1-1/(V1_G1*threadsG1/threads + V1_G2*threadsG2/threads);
...
function ap=access_probability(x,i,d,n,c)
